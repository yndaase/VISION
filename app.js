...
function renderAll() {
  ...
  const filtered = (currentMode === 'objective' && currentFilter !== 'all' && !isMockActive)
    ? activeQuestions.filter(q => q.difficulty === currentFilter)
    : (isMockActive ? QUESTIONS : (currentMode === 'objective' ? activeQuestions : QUESTIONS));

  filtered.forEach((q, idx) => {
    const card = (currentMode === 'objective') ? buildCard(q, idx) : buildTheoryCard(q, idx);
...
function buildCard(q, idx) {
  ...
  div.innerHTML = `
    <div class="q-card-header">
      ...
      <div class="q-tags">
        <span class="tag-context">${q.contextIcon || '📝'}</span>
...
function handleTheorySubmit(qId) {
  ...
  if (!input) { alert("Please provide an answer first."); return; }
  ...
  theoryGrades[qId] = { score: result.score, feedback: result.feedback, userText: input };
...
function markSubjective(input, q) {
  ...
  let commentary = "";
  if (finalScore >= 8) commentary = "Excellent logical flow. You captured all key requirements of the question.";
  else if (finalScore >= 5) commentary = "Good attempt. You identified the main concepts but missed some specific technical details in your explanation.";
  else commentary = "Try to be more specific. Ensure you mention all the core technical components outlined in the mark scheme.";
...
function submitExam() {
  ...
  QUESTIONS.forEach(q => {
    if (q.options) { // MCQ
      ...
      theoryGrades[q.id] = { score: result.score, feedback: result.feedback, userText };
    }
  });
...
function autoReplenish(oldId) {
  ...
  const nextQ = QUESTIONS.find(q => !usedIds.includes(q.id) && !activeQuestions.find(aq => aq.id === q.id));
...
function applyCardFeedback(q, chosen, isCorrect) {
  ...
  if (msg) {
    ...
    msg.innerHTML = isCorrect ? '✅ Correct! Well done.' : `❌ Incorrect. The correct answer is <strong>${q.correct}</strong>`;
  }
...
function updateProgress() {
  ...
  const done = (currentMode === 'objective') 
...
function checkAllAnswered() {
  if (Object.keys(answered).length === QUESTIONS.length && currentMode === 'objective') {
...
function showResults() {
  ...
  document.getElementById('resultsTitle').textContent = `${correct}/${total} — ${pct}%`;
